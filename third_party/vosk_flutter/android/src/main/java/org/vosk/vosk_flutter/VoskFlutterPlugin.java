    ...
    try {
      switch (call.method) {
        ...
        case "recognizer.acceptWaveForm": {
          ...
          // acceptWaveForm() decodes audio synchronously and can take a
          // noticeable amount of time per chunk, especially with larger
          // models. Called inline, that blocks this handler's thread (the
          // Android platform/main thread that Flutter's own UI also runs
          // on), freezing the whole app for the duration of transcription.
          // Run it on TaskRunner's background executor instead, the same
          // pattern already used for model loading above, and post the
          ...
          }, result::success, (exception) -> result.error("ACCEPT_WAVEFORM_ERROR", exception.getMessage(), null));
        }
        break;
        ...
        case "recognizer.getFinalResult": {
          ...
          // Flushing the final result re-decodes any buffered audio and can
          // be slow with larger models; keep it off the main thread too.
          Recognizer finalRecognizer = getRecognizerById(recognizerId);
          new TaskRunner().executeAsync(
              finalRecognizer::getFinalResult,
              result::success,
              (exception) -> result.error("GET_FINAL_RESULT_ERROR", exception.getMessage(), null));
        }
        break;
